...
SOFTENING = 0.03    # Prevents divide by zero
...
uniform float uPointSize; // Particle size
out vec3 vColor; // output color
...
in vec3 vColor;  // Recieved color from vertex shader
out vec4 fColor; // output color

void main() {

    // find coordinates inside point sprite
    vec2 p = gl_PointCoord * 2.0 - 1.0;

    // find distance from center and discard
    float r2 = dot(p, p);
    if (r2 > 1.0) discard;
